...
foreach ($files as $f){
  $fullPath = $path.'/'.$f;
  if(!is_file(fixPath($fullPath)) || ($type=='pdf' && $type=='PDF' && $type=='doc' && $type=="docx" && $type=="xlsx" && $type=="xls"  && $type=='pdf' && $type == 'image' && !RoxyFile::IsImage($f)) || ($type == 'flash' && !RoxyFile::IsFlash($f)))
    continue;
